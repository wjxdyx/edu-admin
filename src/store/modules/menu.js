import { deepClone, resolveRoutePath, isArray } from '@/util'
...
function hasPermission(permissions, route) {
    ...
    if (route.meta && route.meta.auth) {
        console.log(typeof route.meta.auth)
        isAuth = permissions.some(auth => {
            if (typeof route.meta.auth == 'string') {
                return route.meta.auth === auth
            } else if (isArray(route.meta.auth)) {
                console.log('first.level.array', auth)
                return route.meta.auth.some(routeAuth => {
                    console.log('first.level-2', auth, routeAuth)
                    return routeAuth === auth
                })
            } else {
                let routeMetaAuthKeys = Object.keys(route.meta.auth)
                return routeMetaAuthKeys.some(routeAuth => {
                    console.log('first.level.object', auth, routeAuth)
                    return routeAuth === auth
                })
            }
        })
